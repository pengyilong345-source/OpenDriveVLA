"""Asynchronous chase-camera encoder using ffmpeg.

cv2 is NOT available in the carla37 environment, so we pipe raw BGR frames to
an ffmpeg subprocess configured to encode an MP4 (H.264, yuv420p). A bounded
queue + worker thread guarantees the synchronous CARLA control loop is
never blocked by encoding.

Frames are submitted as BGR uint8 arrays keyed by carla_frame.
"""
...
class AsyncChaseEncoder:
    """One chase-camera stream per episode. Pipes BGR frames to ffmpeg."""
