...
当前 `main` 已包含经过验证的 Bench2Drive 数据筛选、下载、流式转换与校验工具。
CARLA 自采样脚本及场景配置尚未完成审查，目前不包含在 `main` 中；待检查完成后，
将通过独立 PR 提交评审。
...
- `main` 提供经过本地全量验证的 Bench2Drive 150k 数据处理工具链。
- 团队成员可以依据固定筛选清单，在本机按类别下载并流式转换相同数据。
- 数据本体不进入 Git，生成目录应保存在仓库之外或被 `.gitignore` 排除。
- CARLA 自采样代码将在完成场景触发、传感器同步、资源清理和可复现性检查后，
  通过独立 PR 提交评审。
