...
def validate(root: Path, annotation: Path) -> list[str]:
    ...
    check(isinstance(event, dict), "event status must be an object", errors)
    if isinstance(event, dict):
        check(
            event.get("type") in EVENT_TYPES,
            "invalid event type",
            errors,
        )
        check(isinstance(event.get("required"), bool), "event.required must be boolean", errors)
        check(isinstance(event.get("triggered"), bool), "event.triggered must be boolean", errors)
        check(isinstance(event.get("success"), bool), "event.success must be boolean", errors)
        check(isinstance(event.get("collision"), bool), "event.collision must be boolean", errors)
        if data.get("scenario_type") == 3:
            check(event.get("required") is True, "extreme emergency must require a semantic event", errors)
            check(event.get("triggered") is True, "extreme emergency event was not triggered", errors)
            check(event.get("state") == "completed", "extreme emergency event did not complete", errors)
            check(event.get("success") is True, "extreme emergency event failed acceptance criteria", errors)
            check(isinstance(event.get("event_actor_id"), int), "extreme emergency event actor is missing", errors)
            check(event.get("collision") is False, "collision occurred during extreme emergency event", errors)
            if "event_actor_collision" in event:
                check(event.get("event_actor_collision") is False, "event actor collided during emergency", errors)
            if "event_evidence_success" in event:
                check(event.get("event_evidence_success") is True, "emergency event lacks completion evidence", errors)
            if "response_detected" in event:
                check(event.get("response_detected") is True, "ego emergency response was not detected", errors)
            if "response_latency_ok" in event:
                check(event.get("response_latency_ok") is True, "ego physical response exceeded configured window", errors)
            if "trigger_distance_ok" in event:
                check(event.get("trigger_distance_ok") is True, "event trigger distance missed acceptance range", errors)
